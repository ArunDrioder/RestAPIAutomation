...
//                .then().assertThat().statusCode(200).body("msg", equalTo("Address successfully updated"));
        ...
        //This is for reading the json (static json file) from the project local path
        RestAssured.baseURI = "https://rahulshettyacademy.com";
        String responseString = given().log().all().queryParam("key", "qaclick123").
                header("Content-Type", "application/json").
                body( new String(Files.readAllBytes(Paths.get("C:\\Users\\arunp\\IdeaProjects\\RestAPIAutomation\\src\\test\\java\\Files\\AddPlace.json")))).when().post("maps/api/place/add/json").
                then().assertThat().statusCode(200).body("scope", equalTo("APP"))
                .header("Server", "Apache/2.4.52 (Ubuntu)").extract().response().asString();
